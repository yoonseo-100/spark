...
window.onload = function() {
    ...
    if (loggedInUser) {
        ...
        if (logoutBtn) logoutBtn.style.display = 'flex';  // 로그아웃 버튼 표시
        if (loginBtn) loginBtn.style.display = 'none';   // 로그인 아이콘 숨기기
    } else {
        ...
        if (loginBtn) loginBtn.style.display = 'flex';   // 로그인 아이콘 표시
    }
};
